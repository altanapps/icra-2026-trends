...
Inside those papers: NVIDIA's flagship GR00T falls from 90% to 4.5% the moment tasks go multi-step. Humanoids, widely reported as the most-funded category in robotics, sit at 3.5% of the research. A model that hallucinates force signals from vision beats the physical force sensor it was trained to imitate.
...
Two years ago the debate was diffusion policy vs. ACT vs. plain behavior cloning. That debate is over. Nobody at ICRA 2026 is asking whether generative action policies work. They're the assumed default: diffusion appears in 173 of 2,951 abstracts, 55 papers build on diffusion policies directly, flow matching shows up in 28. And the papers are about patching, not proving. One hits 95% of state-of-the-art performance at 5% of the training time. Another matches diffusion planners at roughly 100x faster inference. Flow matching is eating diffusion from below.
...
The 2023 dream was that one big model would absorb the whole robot, end to end. The models are certainly everywhere: LLMs appear in 132 abstracts, VLAs in 70. But the 2026 reality, consistent across every slice of the conference, is that foundation models handle the semantics while classical machinery keeps the guarantees. The LLM proposes a plan; a symbolic planner or scene-graph verifier checks it before the robot moves. The model is in the loop, but firewalled.

And here's the part nobody tweets: the same conference published the sharpest evidence yet that VLAs are fragile. A single adversarially textured object cut state-of-the-art VLA success rates by 31–40%. Visual clutter alone cut up to 34%. NVIDIA's flagship GR00T fell from 90% to 4.5% when tasks went multi-step (measured by independent evaluators, not by NVIDIA). And it isn't just exotic attacks. Ordinary observation shift from a preceding skill drops OpenVLA by 54%, and no standard remedy works.
...
You can tell it's settled because the papers are engineering papers now, about pruning for embedded compute, dynamic scenes, and physics coupling. The arguments stopped. The engineering started.

## 5. The classical-vs-learning war ended in a merger

Reinforcement learning is the #2 topic of the entire conference (250 papers). And yet model-predictive control shows up in 149 abstracts and control-barrier-function safety wrappers in 48. The classical toolbox didn't go anywhere. A racecar held stable above 160 mph with zero learning. A GPU reimplementation of a classical solver beat the standard tools by up to 700% on single-rigid-body MPC. A full-size humanoid walked in real time on plain model-predictive control, no policy network in sight. An MCTS planner beat learned planners over 500+ public-road miles, and a symbolic-planning stack beat a fine-tuned VLA 95% to 34%, at roughly 100x less training energy. Learning's retreat is sharpest in driving, where rule-based baselines keep beating learned planners under realistic traffic.

Neither side won, exactly. The fastest-growing recipe is learning inside structure: RL corrects a model-based controller's output, and safety certificates get baked into training instead of bolted on at runtime. Learned components propose; classical control disposes.

## 6. Robots are finally learning to feel

Vision-only manipulation hit a wall, and the field said so out loud. There are 86 papers on force and tactile sensing, hardware built specifically to capture touch during human demonstrations, gloves under $700, and tactile simulators running 4,096 parallel training environments on one consumer GPU.
...
**Security.** Out of 2,951 papers, the ones treating robots as attack surfaces number in the single digits. One of them is an internet-wide scan showing deployed ROS 2 robots are fingerprintable and exploitable in default configurations. Robots are shipping; security research isn't.
...
**Energy and cost.** The papers that meter what anything costs in joules or dollars can be counted on one hand. This in a field whose products have to pencil out.
...
Meanwhile medical robotics, at 111 papers, is bigger than humanoids, and it's quietly moving from perception toward closed-loop autonomous surgery. Almost nobody outside the field is talking about it.

## The humanoid recipe, since that's where the money is

The field has converged hard on how to make a humanoid walk in 2026:
...
The demos got genuinely absurd: a humanoid sustained up to 106 consecutive table-tennis shots against a human opponent. The walking problem is converging. The useful-work problem is next.
